...
class GazeboBridge : public rclcpp::Node {
   public:
    GazeboBridge() : Node("GazeboBridge") {
        ...
        this->setModelStateRequest->model_state.model_name = simulation_config["vehicle_model_name"];
        currentTwist.linear.x = currentTwist.linear.y = currentTwist.linear.z = currentTwist.angular.x = currentTwist.angular.y =
            currentTwist.angular.z = 0;
    }

   private:
    std::shared_ptr<gazebo_msgs::srv::GetModelState::Response> getModelState(const std::string &model_name) {
        this->getModelStateRequest->model_name = model_name;
     ...
     */
    std::pair<std_msgs::msg::Int32, std_msgs::msg::Int32> pingerStatus(const std::string &pinger_name, const float &yaw = 0) {
    ...
    // Service containers
    std::shared_ptr<gazebo_msgs::srv::GetModelState::Request> getModelStateRequest;
    std::shared_ptr<gazebo_msgs::srv::SetModelState::Request> setModelStateRequest;
    // Other
