...
{
    RCL_INFO("March: %f", goal->march);
    RCL_INFO("Lag: %f", goal->lag);
    RCL_INFO("Yaw: %i", goal->yaw);
    ...
    {
        RCL_ERROR("Unable to set horizontal move: %s", serviceCall.response.message.c_str());
        actionServer.setAborted(stingray_movement_msgs::HorizontalMoveResult(),
    ...
    {
        RCL_INFO("Success set horizontal move");
    ...
    if (goal->check_yaw)
    {
        ...
        while (true)
        {
            ...
            RCL_INFO("Current yaw: %d, desired yaw: %d", currentYaw, desiredYaw);
